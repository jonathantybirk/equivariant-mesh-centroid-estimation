...
class MultiCGLayer(nn.Module):
    ...
    def _apply_cg_layer(
        ...
        layer_idx,
        edge_index,
        f,  # not splited by l value
        d,
        a,  # already splited by l value
    ):
        """Apply one CG layer"""
        device = f.device
        num_nodes = f.shape[0]
        ...
        f = _split_sh_features_tensor(f, self.max_sh_degree)

        # Initialize output messages
        output = []
        for l_value in range(self.max_sh_degree + 1):
            base_dim = 2 * l_value + 1  # Compute dimension from l value
            for channel in range(self.multiplicity):
                output.append(torch.zeros(num_edges, base_dim, device=device))
        ...
        for edge_sh_degree, node_irrep_idx, out_irrep_idx in valid_combinations:
            ...
            for in_channel in range(self.multiplicity):
                for out_channel in range(self.multiplicity):
                    ...
                    # Get source and target node features for this edge
                    source_features = f[node_irrep_idx][in_channel][
                        source_idx
                    ]  # [num_edges, irrep_dim]
                    target_features = f[out_irrep_idx][out_channel][
                        target_idx
                    ]  # [num_edges, irrep_dim]
                    h = torch.cat(
                        [source_features, target_features, d], dim=1
                    )  # [num_edges, 2*irrep_dim + 1]

                    # CG tensor product
                    msg = torch.einsum("ijk,ei,ej->ek", cg_coeffs, a[edge_sh_degree], h)
                    ...
                    output[output_channel_idx] += gate.unsqueeze(-1) * msg

        # Aggregate messages
        aggregated_channels = []
        for channel_idx in range(len(output)):
            base_dim = output[channel_idx].shape[1]
            agg_tensor = torch.zeros(num_nodes, base_dim, device=device)
            agg_tensor.index_add_(0, target_idx, output[channel_idx])
...
class EquivariantGNN(BaseModel):
    ...
    def __init__(
        self,
        message_passing_steps=1,  # Reduced from 3 to 1 for equivariance
        ...
        # FIXED: Build final MLP for invariant features from equivariant representations
        final_layers = []
        prev_dim = len(self.base_l_values)  # One invariant feature per l value
        ...
        # FIXED: Initialize final layer with larger weights for meaningful predictions
        with torch.no_grad():
            # Use larger initialization to avoid dead start
            self.final_mlp[-1].weight.data *= 2.0  # Increase from 0.1 to 2.0
            # Add small random bias to break symmetry
            self.final_mlp[-1].bias.data.uniform_(-0.1, 0.1)
    ...
    def forward(self, x, edge_index, r, batch=None, node_pos=None):
        """Ultra-fast forward pass with proper equivariance"""
        if self.debug:
            start_time = time.time()

        # TODO: we need to set f first
        ...
        a = spherical_harmonics(self.max_sh_degree, r, normalize=True)
        a = _split_sh_features_tensor(a, self.max_sh_degree)
        d = torch.norm(r, dim=1, keepdim=True)

        # Message passing with proper equivariance - NO skip connections for equivariance
        ...
        for l_idx, l_value in enumerate(self.base_l_values):
            ...
            for channel in range(self.multiplicity):
                ...
                l_channels.append(f[:, start_idx:end_idx])
                start_idx = end_idx
        ...
        if batch is not None:
            ...
            for i, b in enumerate(unique_batches):
                ...
                weighted_nodes = torch.sum(
                    batch_weights.unsqueeze(-1) * batch_positions, dim=0
                )  # [3]

                centroid_predictions.append(weighted_nodes)
            ...
            weighted_nodes = torch.sum(weights.unsqueeze(-1) * node_pos, dim=0)  # [3]
            geometric_center = node_pos.mean(dim=0)  # [3]

            # FIXED: Use more of geometric center for stability
            final_com = 0.8 * geometric_center + 0.2 * weighted_nodes
...
def _split_sh_features_tensor(self, edge_attr, max_sh_degree):
