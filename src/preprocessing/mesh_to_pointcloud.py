...
def preprocess_mesh_to_unit_sphere(mesh_obj):
    ...
    final_mesh_centroid = (
        mesh_centroid + translation
    )  # This equals translation since mesh_centroid was [0,0,0]

    return mesh_obj, original_mesh_centroid, max_dist, final_mesh_centroid


def generate_systematic_camera_positions(distance=2.0):
    """
    Generate camera positions with first camera random and others on opposite side.

    Args:
        num_cameras: Number of cameras
        distance: Distance from origin (fixed at 2.0)
        angle_noise_deg: Angular noise in degrees (unused in simplified version)

    Returns:
        numpy.ndarray: Camera positions (K, 3)
    """
    # Step 1: Place first camera at random position
    first_direction = geometry.random_point_on_sphere(radius=distance)

    camera_positions = np.array([first_direction, -first_direction])
...
def visualize_first_n_pointclouds(cfg, n_visualize=3):
    ...
    for i, obj_file in enumerate(obj_files):
        ...
        try:
            ...
            # Apply random rotation to remove orientation bias
            mesh_obj = apply_random_rotation_to_mesh(mesh_obj)
            ...
            if not all(
                os.path.exists(p) for p in [mesh_centroid_path, pointcloud_path]
            ):
...
def process_mesh_optimized(args):
    ...
    """
    mesh_path, cfg, obj_idx, total_objs, seed = args
    np.random.seed(seed)
    ...
    try:
        ...
        # Apply random rotation to remove orientation bias
        original_mesh = apply_random_rotation_to_mesh(original_mesh)

        # Stage 1: Preprocess mesh to unit sphere with random translation
        processed_mesh, original_centroid, mesh_scale_factor, final_mesh_centroid = (
            preprocess_mesh_to_unit_sphere(original_mesh)
        )
        ...
        for sample_idx in range(num_samples):
            ...
            if all_points_by_camera:
                ...
                normalized_pointcloud, pc_centroid, pc_scale_factor = (
                    normalize_pointcloud_to_unit_sphere(raw_pointcloud)
                )
                ...
                camera_target_origin = np.array(
                    [0.0, 0.0, 0.0]
                )  # The origin where cameras aimed
                ...
                if cfg.get("save_individual_cameras", False):
                    ...
                    for cam_points in all_points_by_camera:
                        ...
                        if len(cam_points) > 0:
                            ...
                            normalized_cam = (
                                centered_cam / pc_scale_factor
                                if pc_scale_factor > 0
                                else centered_cam
                            )
...
def save_object_data_batch(object_data_list, cfg):
    ...
    for object_data in object_data_list:
        ...
        obj_output_dir = os.path.join(original_cwd, cfg.output_dir, obj_name)
...
def update_metadata_batch(object_data_list, cfg):
    ...
    for object_data in object_data_list:
        ...
        for sample_data in object_data["samples"]:
            ...
            avg_camera_distance_pc = np.mean(
                [np.linalg.norm(cam_pos) for cam_pos in camera_positions_pc]
            )
...
def process_all_meshes_optimized(cfg):
    ...
    args_list = [
        (mesh_path, cfg.preprocessing.lidar, idx, len(obj_files), seed + idx)
            ...
            if object_data is not None:
                ...
                if visualize_first_n > 0 and processed_count < visualize_first_n:
                    try:
                        ...
                        mesh_path = args[0]
                        mesh_obj = mesh.load_mesh(mesh_path, debug=False)
                        # TODO: DOES not work since we need to apply the same random rotation to the mesh
                        # visualize_during_processing(
                        #     mesh_obj,
                        #     object_data,
                        #     object_data["samples"][0]["mesh_centroid_pc_coords"],
            ...
            for i in tqdm(
                ...
                if object_data_list:
                    ...
                    if cfg.preprocessing.lidar.save:
                        save_object_data_batch(
                            object_data_list, cfg.preprocessing.lidar
                        )
    ...
    if visualize_first_n > 0:
        if n_processes == 1:
            ...
            print(
                "Parallel processing was used - showing post-processing visualization..."
            )
            visualize_first_n_pointclouds(cfg.preprocessing.lidar, visualize_first_n)


def visualize_during_processing(
    mesh_obj, pointcloud_data, mesh_centroid, obj_name, obj_idx, total_objs
):
    """
    ...
    if pointcloud_data["samples"]:
        ...
        print(
            f"Real-time visualization {obj_idx + 1}/{total_objs}: {obj_name} ({len(pointcloud)} points)"
        )

        # Call the existing visualization function with pointcloud coordinate system data
        visualize_pointcloud_with_mesh(
            mesh_obj, pointcloud, mesh_centroid_pc, obj_name, camera_positions
        )
