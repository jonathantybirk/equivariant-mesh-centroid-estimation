...
Usage:
    ...
    python visualize_pointclouds.py --save  # Save visualizations to results directory
"""
...
try:
    ...
    MESH_SUPPORT = False
    print(
        "Warning: Mesh visualization dependencies not available. --show_mesh option will be disabled."
    )
...
def visualize_single_pointcloud(
    data, object_name, show_individual_cameras=True, save_path=None
):
    """
    ...
    ax.scatter(
        pointcloud_centroid[0],
        pointcloud_centroid[1],
        pointcloud_centroid[2],
        c="red",
        s=200,
        alpha=1.0,
        marker="o",
        label="Pointcloud Centroid",
        edgecolors="darkred",
        linewidth=2,
    )
...
def visualize_multiple_pointclouds(
                ...
                for j, cloud in enumerate(individual_clouds):
                    if len(cloud) > 0:
                        ax.scatter(
                            ...
                            alpha=0.6,
                            label=(
                                f"Cam{j+1}" if j < 3 else None
                            ),  # Only label first 3 cameras for space
                        )
            else:
                # Plot combined pointcloud
                ax.scatter(
    ...
    if save_path:
        fig.savefig(save_path, dpi=300, bbox_inches="tight")
        print(f"Saved multi-visualization to {save_path}")
...
def analyze_pointcloud_statistics(pointcloud_dirs, object_names, sample_size=100):
    ...
    for pointcloud_dir, object_name in zip(sample_dirs, sample_names):
        try:
            ...
            stats["pc_mesh_distances"].append(
                np.linalg.norm(pointcloud_centroid - mesh_centroid)
            )

            if camera_positions is not None:
                stats["camera_counts"].append(len(camera_positions))
                avg_cam_dist = np.mean(
                    [np.linalg.norm(cam - mesh_centroid) for cam in camera_positions]
                )
    ...
    if successful_loads > 0:
        ...
        if stats["camera_counts"]:
            ...
            print(f"\nCamera Statistics:")
            print(
                f"  Cameras per object: {np.mean(cam_counts):.1f} ± {np.std(cam_counts):.1f}"
            )
            print(
                f"  Average camera distance: {np.mean(cam_dists):.3f} ± {np.std(cam_dists):.3f}"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Visualize 3D pointclouds with centroids and cameras"
    )
    parser.add_argument(
        "--pointcloud_dir",
        type=str,
    ...
    parser.add_argument(
        "--random", action="store_true", help="Randomize the order of pointclouds"
    )
    parser.add_argument(
        "--num_objects",
        type=int,
    ...
    parser.add_argument(
        "--single", action="store_true", help="Show only one pointcloud"
    )
    parser.add_argument(
        "--analyze",
        action="store_true",
    ...
    parser.add_argument(
        "--save", action="store_true", help="Save visualizations to results directory"
    )
    ...
    if args.multi:
        ...
        if args.show_mesh:
            print(
                "Note: Mesh visualization is not supported in multi-grid mode. Use --single for mesh display."
            )
        fig = visualize_multiple_pointclouds(
            pointcloud_dirs,
            object_names,
            max_display=6,
            show_individual_cameras=args.individual_cameras,
            save_path=RESULTS_DIR / "multi_visualization.png" if args.save else None,
        )
        plt.show()
        ...
        if pointcloud_dirs:
            ...
            save_path = None
            if args.save:
                save_path = RESULTS_DIR / f"{object_names[0]}_single_visualization.png"

            if args.show_mesh:
                fig, ax = visualize_single_pointcloud_with_mesh(
                    data, object_names[0], args.individual_cameras, args.mesh_dir
                )
                if args.save:
                    fig.savefig(save_path, dpi=300, bbox_inches="tight")
                    print(f"Saved visualization to {save_path}")
            else:
                fig, ax = visualize_single_pointcloud(
                    data, object_names[0], args.individual_cameras, save_path
                )
            plt.show()
        ...
        for pointcloud_dir, object_name in zip(pointcloud_dirs, object_names):
            try:
                ...
                save_path = None
                if args.save:
                    save_path = RESULTS_DIR / f"{object_name}_visualization.png"

                if args.show_mesh:
                    fig, ax = visualize_single_pointcloud_with_mesh(
                        data, object_name, args.individual_cameras, args.mesh_dir
                    )
                    if args.save:
                        fig.savefig(save_path, dpi=300, bbox_inches="tight")
                        print(f"Saved visualization to {save_path}")
                    ...
                    fig, ax = visualize_single_pointcloud(
                        data, object_name, args.individual_cameras, save_path
                    )
                plt.show()
            except Exception as e:
...
def visualize_single_pointcloud_with_mesh(
    data, object_name, show_individual_cameras=True, mesh_dir="data/meshes"
):
    """
    ...
    # Add center of mass
    ax1.scatter(
        mesh_center_of_mass[0],
        mesh_center_of_mass[1],
        mesh_center_of_mass[2],
        c="red",
        s=100,
        alpha=1.0,
